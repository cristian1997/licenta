#ifndef EXPONENTIALWEAKLEARNER_H
#define EXPONENTIALWEAKLEARNER_H

#include <iostream>
#include "weaklearnerprovider.h"

class ExponentialWeakLearnerProvider : public WeakLearnerProvider {
public:
	std::pair<WeakLearner, bool> getWeakLearner(
		...
		for(int i = 0; i < numOfFeatures; ++i) {
			...
			for(int j = 0; j < data.size(); ++j) {
				...
				// std::cout << i << ' ' << currThreshold << ' ' << std::min(error, 1 - error) << std::endl;
			}
		}
	...
	void updateWeights(const WeakLearner &weakLearner, const std::vector<std::vector<double>> &data,
		...
		for(int i = 0; i < w.size(); ++i) {
			w[i] *= exp(-weakLearner.getAlpha() * weakLearner.getLabel(data[i]) * data[i].back());
			s += w[i];
		}
